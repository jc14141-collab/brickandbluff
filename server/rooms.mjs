import {smallBlind,entryChips} from '../dist/poker-stakes.mjs';
import {replayRouletteWagers} from '../dist/roulette-wagers.mjs';
...
export function createRoom(auth,data,now=Date.now()){assert(MODES.includes(data.mode),'未知游戏');const cap=['guandan','bigtwo'].includes(data.mode)?4:Number(data.capacity??4),min=data.solo===true&&['craps','roulette','blackjack'].includes(data.mode)?1:data.mode==='poker'?4:2,max=data.mode==='poker'?10:data.mode==='blackjack'?6:['craps','roulette'].includes(data.mode)?8:4;assert(Number.isInteger(cap)&&cap>=min&&cap<=max,'人数设置不正确');const p=profile(data);return{id:uuid(),host:auth,mode:data.mode,smallBlind:data.mode==='poker'?smallBlind(data.smallBlind):10,value:normalizeValue(data.value),capacity:cap,status:'lobby',seats:[{...p,id:uuid(),auth,bot:false,ready:true,bank:2000,seen:now}],round:0,game:null,seq:0,created:now,expires:now+86400000,nextAt:0,requests:[],message:'邀请朋友，或添加电脑补位'} }
...
function readyRound(r,now){assert(r.seats.length>=minimumPlayers(r),'人数不足：掼蛋与大老二需要四人，其他朋友房间至少两人');assert(r.seats.every(s=>s.bot||s.ready),'请等待所有玩家准备');assert(r.mode==='guandan'||r.seats.every(s=>s.bot||s.bank>0),'有玩家筹码不足，请联系管理员发放');r.seats.forEach(s=>delete s.waiting);r.status='playing';r.round++;r.seq++;r.message='游戏开始 · 经典规则';r.seats.forEach(s=>{if(s.bot&&!s.cashOwner){if(r.mode==='bigtwo'){const need=Math.max(2000,normalizeValue(r.value??DEFAULT_CARD_VALUE)*200);if(s.bank<need)s.bank=need}else if(r.mode==='poker'&&s.bank<(r.smallBlind??10)*2)s.bank=entryChips(r.smallBlind);else if(r.mode!=='poker'&&s.bank<(r.mode==='blackjack'?50:20))s.bank=2000}});
 startSkills(r);
 if(r.mode==='poker'){r.seats.forEach(s=>{if(s.bot&&!Number.isInteger(s.aiProfile))s.aiProfile=randomProfile(secureRandom)});r.game=new Poker(r.seats.map(s=>Math.floor(s.bank)),{smallBlind:r.smallBlind??10,names:r.seats.map(s=>s.name),dealer:(r.round-1)%r.seats.length,rng:secureRandom});r.nextAt=now+1100;r.deadline=now+45000}
 ...
 if(r.mode==='bigtwo'){r.seats.forEach(s=>s.autoplay=false);r.match??=new BigTwoMatch(secureRandom,r.value);r.match.random=secureRandom;r.match.value=storedValue(r.value??DEFAULT_CARD_VALUE);r.game=r.match.start();r.nextAt=now+1100;r.deadline=now+60000}
...
export function command(r,auth,data,now=Date.now()){const kind=data.kind;let i=seatOf(r,auth);if(kind==='join'){if(i>=0){r.seats[i].seen=now;return}assert(!r.solo,'这是单人牌桌');assert(r.seats.length<r.capacity,'房间已满');r.seats.push({...profile(data),id:uuid(),auth,bot:false,ready:r.status!=='lobby',waiting:r.status!=='lobby',bank:2000,seen:now});r.seq++;return}assert(i>=0,'你尚未加入房间');r.seats[i].seen=now;const isHost=auth===r.host,seat=r.seats[i];
 ...
 if(kind==='autoplay'){assert(r.mode==='bigtwo'&&r.status==='playing'&&!seat.waiting,'只能在大老二局内切换托管');assert(typeof data.enabled==='boolean','托管状态无效');seat.autoplay=data.enabled;r.seq++;return}
 ...
 else if(r.mode==='bigtwo'){assert(!seat.autoplay,'请先取消托管');r.game.act(i,data.ids??[]);resetTurn(r,now)}
...
export function tick(r,now=Date.now()){if(!r.closeAfterRound&&r.status==='roundEnd'&&now>=(r.reviewUntil??0)&&r.seats.some(s=>!s.bot)&&r.seats.every(s=>s.bot||s.ready)){readyRound(r,now);return true}if(r.status!=='playing')return false;const g=r.game;if(now<r.nextAt)return false;if(botSkill(r,now)){r.nextAt=now+1000;return true;}
 ...
 if(r.mode==='bigtwo'){const i=g.turn;if(r.seats[i].bot||r.seats[i].autoplay||now>=r.deadline){const m=chooseBigTwoMove(g.snapshot(i));g.act(i,m?.cards.map(c=>c.id)||[]);resetTurn(r,now);return true}}
...
export function snapshot(r,auth,now=Date.now()){const i=seatOf(r,auth),room={id:r.id,mode:r.mode,smallBlind:r.smallBlind??10,entryChips:r.mode==='poker'?entryChips(r.smallBlind):0,value:storedValue(r.value??DEFAULT_CARD_VALUE),solo:r.solo??false,skill:skillView(r,i),capacity:r.capacity,status:r.status,round:r.round,seq:r.seq,you:i,host:r.host===auth,hostId:r.seats.find(s=>s.auth===r.host)?.id??null,expires:r.expires,deadline:r.deadline??0,serverNow:now,endedAt:r.endedAt??0,reviewUntil:r.reviewUntil??0,message:r.message,seats:r.seats.map((s,n)=>({...chipBalance(r,n),id:s.id,name:s.name,role:s.role,seed:s.seed,bot:s.bot,autoplay:!!s.autoplay,style:r.mode==='poker'&&s.bot?PROFILES[s.aiProfile]?.label:null,ready:s.ready,bank:s.bank,connected:s.bot||now-s.seen<25000}))};room.closeAfterRound=!!r.closeAfterRound;room.minimumPlayers=minimumPlayers(r);room.waiting=!!r.seats[i]?.waiting;room.waitingPlayers=r.seats.filter(s=>s.waiting).map(s=>({name:s.name,role:s.role}));if(i<0||!r.game||room.waiting)return room;room.seats=room.seats.filter((_,n)=>!r.seats[n].waiting);const g=r.game;
