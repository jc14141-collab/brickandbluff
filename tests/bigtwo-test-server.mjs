...
ok('房间默认牌值 50',r.value===50,r.value);
...
ok('越界牌值夹到 500',r.value===500,r.value);
command(r,'auth-0',{kind:'value',value:7},1050);
ok('过小牌值夹到 50',r.value===50,r.value);
command(r,'auth-0',{kind:'value',value:55},1050);
ok('牌值吸附到 50',r.value===50,r.value);
command(r,'auth-0',{kind:'value',value:50},1050);
ok('牌值设为 50',r.value===50,r.value);

r.seats.forEach(s=>s.bank=100000);command(r,'auth-0',{kind:'start'},1100);
...
ok('快照下发 value',snapshot(r,'auth-0',1200).value===50);
ok('game.value 与房间一致',snapshot(r,'auth-0',1200).game.value===50);
ok('游戏中不能改牌值',(()=>{try{command(r,'auth-0',{kind:'value',value:30},1200);return false}catch{return true}})());
ok('改牌值失败后牌值不变',r.value===50,r.value);
...
ok('结算用了房间牌值 50',res.value===50,res.value);
...
ok('第二副沿用同一牌值',r.match.value===50,r.match.value);
