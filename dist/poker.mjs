...
export class Poker {
 constructor(stacks,{dealer=0,names=['你','Atlas','Vega','Kite'],rng=Math.random}={}){
  if(stacks.length<2||stacks.length>10||stacks.some(n=>!Number.isInteger(n)||n<20))throw Error("牌桌需要 2–10 位玩家，每位至少 20 筹码");
  this.count=stacks.length;dealer=((dealer%this.count)+this.count)%this.count;
  this.players=stacks.map((chips,i)=>({name:names[i]??`玩家 ${i+1}`,chips,cards:[],fold:false,total:0,bet:0,status:'',actedAt:null,actedRaise:BIG_BLIND,history:[]}));
  this.deck=deck(rng);this.board=[];this.dealer=dealer;this.street=0;this.current=BIG_BLIND;this.lastRaise=BIG_BLIND;this.done=false;this.actions=[];this.events=[];this.turn=this.count===2?dealer:(dealer+3)%this.count;this.lastAggressor=null;
  ...
  this.post(this.count===2?dealer:(dealer+1)%this.count,SMALL_BLIND,'小盲');this.post(this.count===2?(dealer+1)%this.count:(dealer+2)%this.count,BIG_BLIND,'大盲');
 ...
 legal(i=this.turn){let p=this.players[i],owe=Math.max(0,this.current-p.bet),max=p.bet+p.chips;
  ...
  const min=this.current<BIG_BLIND?BIG_BLIND:this.current+this.lastRaise;
 ...
 act(i,action,target){if(this.done||i!==this.turn||!this.pending.has(i))throw Error('当前不能行动');let p=this.players[i],l=this.legal(i),kind=action,amount=0,old=this.current;
  ...
  if(action==='fold'){p.fold=true;p.status='弃牌'}
   ...
   const increment=old<BIG_BLIND?Math.max(BIG_BLIND,this.current-old):this.current-old;
   if(this.current>=BIG_BLIND&&increment>=this.lastRaise)this.lastRaise=increment;
 ...
 advance(){if(this.done||this.pending.size)return false;this.returnUncalled();if(this.street===3){this.finish();return true}
  ...
  this.current=0;this.lastRaise=BIG_BLIND;this.players.forEach(p=>{p.bet=0;p.actedAt=null;p.actedRaise=BIG_BLIND;if(!p.fold)p.status=p.chips?'':'全下'});
