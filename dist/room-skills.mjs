...
export function renderRoomSkills(view,r){
 ...
 const cost=poker?(role===0?5*(r.smallBlind??10)*2:role===1?10*(r.smallBlind??10)*2:0):role===3?0:Math.round(h.bet*(role===2?.2:.5));can=can&&(poker?own.chips:own.bank)>=cost;if(!poker&&role===3)can=can&&(own.bet?own.bank>=own.bet:own.bank>=2);
