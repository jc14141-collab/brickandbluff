...
export class Craps{
 ...
 amount(k,chip){return Math.ceil(chip/this.unit(k))*this.unit(k)}
 add(k,chip){if(this.rolling)throw Error('骰子停稳后再下注');if(!Object.hasOwn(this.bets,k))throw Error('未知下注区');if(!Number.isFinite(chip)||chip<=0)throw Error('下注金额无效');if(this.point&&['pass','dont'].includes(k))throw Error('点数已建立，下一轮开局再下底线注');if(!this.point&&['come','dcome'].includes(k))throw Error('COME 注在目标点数建立后开放');const n=this.amount(k,chip),error=extraError(this,k,n);if(error)throw Error(error);if(this.bank<n)throw Error('可用筹码不足');this.bank-=n;this.bets[k]+=n}
 ...
 const resolve=(k,m,stay=false)=>{const stake=this.bets[k];if(!stake)return;const profit=stake*m;this.bank+=m>=0?(stay?profit:stake+profit):0;if(!stay||m<0)this.bets[k]=0;events.push({key:k,profit:m<0?-stake:profit})};
 ...
 this.rolling=false;const result={dice,total,oldPoint,point:this.point,events,net:Math.round((this.bank+this.escrow-before)*100)/100};this.history.unshift(result);this.history.length=Math.min(12,this.history.length);return result
 }
}
