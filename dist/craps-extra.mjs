...
export function settleExtras(g,dice,oldPoint,events){const total=dice[0]+dice[1];
 const pay=(k,m,stay=false,fee=0)=>{const stake=g.bets[k]??0;if(!stake)return;const profit=m<0?-stake:Math.round((stake*m-fee)*100)/100;if(m>=0)g.bank+=profit+(stay?0:stake);if(!stay||m<0)g.bets[k]=0;events.push({key:k,profit,commission:fee})};
 for(const c of EXTRA_BETS){const k=c.key,stake=g.bets[k]??0;if(!stake)continue;const o=oddsInfo(g,k);
  ...
  if(k.startsWith('buy')){const n=+k.slice(3);if(oldPoint){if(total===7)pay(k,-1);else if(total===n)pay(k,ratio(n),true,Math.ceil(stake*.05-1e-9))}continue}
  if(k.startsWith('lay')){const n=+k.slice(3);if(total===n)pay(k,-1);else if(total===7)pay(k,1/ratio(n),true,Math.ceil(stake/ratio(n)*.05-1e-9));continue}
